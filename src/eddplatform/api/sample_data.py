...
EVALUATORS = [
    EvaluatorDef(name="金额校验", kind=EvaluatorKind.CUSTOM_CODE, input_field=ContextField.OUTPUT,
                 ...
                 output_type=OutputType.ASSERTION, dimension="正确性", case_refs=["17", "88", "91"]),
    EvaluatorDef(name="条款解释", kind=EvaluatorKind.LLM_JUDGE, input_field=ContextField.OUTPUT,
                 ...
                 model="openai:gpt-5.2", output_type=OutputType.SCORE, threshold=4.0,
                 dimension="文字质量", case_refs=["63"]),
    EvaluatorDef(name="工具调用正确", kind=EvaluatorKind.BUILTIN, builtin_type="HasMatchingSpan",
                 input_field=ContextField.SPAN_TREE, output_type=OutputType.ASSERTION,
                 dimension="轨迹", case_refs=["17"]),
    EvaluatorDef(name="最小调用链路", kind=EvaluatorKind.BUILTIN, builtin_type="MinimalToolChain",
                 input_field=ContextField.SPAN_TREE, threshold=2, output_type=OutputType.ASSERTION,
                 dimension="轨迹", case_refs=["17"]),
    EvaluatorDef(name="延迟阈值", kind=EvaluatorKind.BUILTIN, builtin_type="MaxDuration",
                 ...
                 threshold=3.0, dimension="延迟", case_refs=["91"]),
    EvaluatorDef(name="TTFT达标", kind=EvaluatorKind.BUILTIN, builtin_type="MaxTTFT",
                 input_field=ContextField.TTFT, threshold=0.8, output_type=OutputType.ASSERTION,
                 dimension="TTFT", case_refs=["63"]),
    EvaluatorDef(name="成本达标", kind=EvaluatorKind.BUILTIN, builtin_type="MaxCost",
                 input_field=ContextField.COST, threshold=0.02, output_type=OutputType.ASSERTION,
                 dimension="成本", case_refs=["63"]),
    EvaluatorDef(name="提示词注入检测", kind=EvaluatorKind.CUSTOM_CODE, input_field=ContextField.OUTPUT,
                 rule="未越权 / 未泄露", output_type=OutputType.ASSERTION, dimension="安全"),
]
