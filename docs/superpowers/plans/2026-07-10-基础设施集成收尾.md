...
> demo 从 `tests.release_sample` 取样例是刻意的：本 demo 是"用现成确定性样例演示 live 编排"，不重复造数据。`tests/` 在仓库根，`pip install -e` 后 `pythonpath=["src"]`，`tests` 作为顶层包可被 `python examples/...` 导入（仓库根在 `sys.path[0]`）。若 import 失败，用 `PYTHONPATH=. python examples/temporal_release_demo.py`。

- [ ] **Step 4: 加 demo import smoke 测试（不联网）**

在 `tests/test_temporal_workflow.py` 追加：

```python
def test_demo_is_import_safe_and_builds_request():
    """demo 可安全 import（无顶层副作用）且能构造合法请求——不连 server。"""
    import importlib

    demo = importlib.import_module("examples.temporal_release_demo")
...
> 若 `examples` 不是包导致 import 失败：确认 `examples/__init__.py` 是否需要。仓库现有 `examples/` 无 `__init__.py` 但内部靠脚本直接运行。为让测试能 `import_module("examples.temporal_release_demo")`，本步同时新建空文件 `examples/__init__.py`（仅此用途）。若新建了，`git add examples/__init__.py`。
...
```bash
git add deploy/temporal examples/temporal_release_demo.py tests/test_temporal_workflow.py examples/__init__.py
