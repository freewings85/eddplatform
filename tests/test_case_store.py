...
def test_add_generates_id_and_timestamps(store):
    c = store.add_case("insurance", _case())
...
def test_next_id_is_max_numeric_plus_one(store):
    store.add_case("insurance", _case(id="17"))
    store.add_case("insurance", _case(id="88"))
    c = store.add_case("insurance", _case())
    assert c.id == "89"
...
def test_list_preserves_insertion_order(store):
    for i in ("a", "b", "c"):
        store.add_case("insurance", _case(id=i))
    assert [c.id for c in store.list_cases("insurance")] == ["a", "b", "c"]


def test_systems_are_isolated(store):
    store.add_case("insurance", _case(id="1"))
    store.add_case("cs", _case(id="1"))
    assert len(store.list_cases("insurance")) == 1
    assert len(store.list_cases("cs")) == 1


def test_get_case(store):
    store.add_case("insurance", _case(id="17", name="报价"))
    assert store.get_case("insurance", "17").name == "报价"
    assert store.get_case("insurance", "999") is None


def test_update_keeps_id_created_at_refreshes_updated(store):
    a = store.add_case("insurance", _case(id="17", name="旧"))
    updated = store.update_case("insurance", "17", _case(id="ignored", name="新"))
...
def test_update_missing_raises(store):
    with pytest.raises(KeyError):
        store.update_case("insurance", "999", _case())


def test_delete(store):
    store.add_case("insurance", _case(id="17"))
    store.delete_case("insurance", "17")
    assert store.get_case("insurance", "17") is None
    with pytest.raises(KeyError):
        store.delete_case("insurance", "17")
...
def test_trace_roundtrips(store):
    ...
    store.add_case("insurance", _case(id="17", trace=t))
    got = store.get_case("insurance", "17")
...
def test_export_returns_all(store):
    store.add_case("insurance", _case(id="1"))
    store.add_case("insurance", _case(id="2"))
    assert len(store.export_cases("insurance")) == 2


def test_import_append_upserts_by_id(store):
    store.add_case("insurance", _case(id="17", name="旧"))
    ...
    assert store.get_case("insurance", "17").name == "改"


def test_import_replace_clears_first(store):
    store.add_case("insurance", _case(id="17"))
    store.add_case("insurance", _case(id="88"))
    res = store.import_cases("insurance", [_case(id="900")], mode="replace")
    ids = {c.id for c in store.list_cases("insurance")}
    assert ids == {"900"}
    assert res.total == 1
